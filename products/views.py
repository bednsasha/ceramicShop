...
class CatalogView(TemplateView):
    ...
    FILTER_MAPPING={
       ...
       'min_size':lambda queryset, value:queryset.filter(product_size__value__gte=value),
       'max_size':lambda queryset, value:queryset.filter(product_size__value__lte=value),
    ...
    def get_context_data(self, **kwargs):
        ...
        for param, filter_func in self.FILTER_MAPPING.items():
            ...
            if value:
                if param in ['min_price', 'max_price', 'min_size', 'max_size']:
                    try:
                        value = float(value)
                    except ValueError:
                        continue
                ...
                filter_params[param] = ''
        if any(filter_params.get(p) for p in ['min_size', 'max_size']):
